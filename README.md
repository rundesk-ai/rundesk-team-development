...
- `debugging-code` — Find an unproved cause: reproduce, preserve evidence, isolate the responsible boundary, test one hypothesis at a time, and prove the smallest safe correction, with stack mechanics for Laravel and Herd, Vue, React, Python, and C++.
- `reviewing-code` — Judge a completed change: establish the artifact and base, trace changed contracts to their callers, prove each finding, rank by reachable impact, and issue a defensible readiness verdict.
- `testing-code` — Prove behavior with tests: choose the boundary that contains the risk, make one case trustworthy and deterministic, replace known traps, and prove the test fails without the change it covers.
