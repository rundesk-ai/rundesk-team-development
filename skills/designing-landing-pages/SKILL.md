...
description: Use when designing or reviewing a campaign landing page or focused conversion path, including its message hierarchy, hero, proof, calls to action, forms, responsive states, rendered experience, measurement handoff, or experiment plan. It supplies an evidence-backed page design, prioritized findings, conversion measurement contract, and implementation handoff. Do not use for general application UI, campaign operation, analytics implementation, production code, brand direction, or legal approval.
---
...
Route U.S. lead-transfer and contact-consent gates to the applicable compliance owner. This skill
identifies where approval is needed but does not write or approve consent language.
...
5. a page and flow design describing each region's purpose, content requirement, hierarchy, action,
   responsive constraint, and required state without prescribing production code;
...
8. an implementation handoff naming required content, development, analytics, compliance, and
   verification owners.

Use approved messaging, claims, brand direction, and design-system constraints to specify the page's
visual hierarchy and exact interface wording. Do not invent brand direction or unsupported claims,
edit source, implement events, configure analytics, launch an experiment, or approve legal language.
Inspect the rendered page when tools allow and return the observed design and experience verdict;
development and analytics owners must return their own implementation and measurement proof.
